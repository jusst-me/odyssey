...
export const metadata: Metadata = {
  ...
  description:
    'Tours en activiteiten in Albanië — GetYourGuide feeds als native redactionele kaarten.',
};

export default async function DealsActivitiesPage({ searchParams }: PageProps) {
  const params = await searchParams;
  const region = parseRegionParam(params.regio);
  const page = parseDealsPageParam(params.pagina);
  const basePath = '/deals/activiteiten';

  const filtered = offersByVerticalAndRegion('activiteiten', region);
  const picks = editorPicks('activiteiten').filter(
  ...
  return (
    <main>
      <section
        ...
        className="bg-background border-border border-b py-12 md:py-16"
      >
        <div className="mx-auto max-w-3xl space-y-5 px-6">
          ...
          <p className="text-primary text-xs font-medium tracking-[0.2em] uppercase">
            Ervaringen · GetYourGuide feed
          </p>
          <h1 id="activities-heading" className="text-3xl md:text-4xl">
            Activiteiten &amp; tours
          </h1>
          <p className="text-muted-foreground leading-relaxed">
            Dagtrips, food tours en outdoor — geen GetYourGuide-widget embed, wel dezelfde data in
            magazine-stijl met duur, prijs en ghost CTA.
          </p>
        </div>
      </section>

      <section aria-labelledby="region-filter" className="bg-muted border-border border-b py-6">
          ...
          <div className="mx-auto max-w-6xl space-y-8 px-6">
            <header className="space-y-1">
              ...
              <p className="text-muted-foreground text-sm">
                Tours die we zelf zouden boeken — geen &ldquo;Bestseller&rdquo;-schreeuwlabels
              </p>
            </header>
      ...
      <section aria-labelledby="listing-heading" className="bg-muted py-12 md:py-16">
        <div className="mx-auto max-w-6xl space-y-8 px-6">
          <header className="space-y-1">
            ...
            <p className="text-muted-foreground text-sm">
              {listing.totalItems} tours · pagina {listing.currentPage} van {listing.totalPages}
            </p>
          </header>
          <ul className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
